...
class App extends Component {
  constructor(props) {
    ...
    this.handleOverNightToggle = this.handleOverNightToggle.bind(this);
  ...
  calculate() {
    const dayPerDiem = 686;
    const singleDayPerDiemShort = 0;
    const singleDayPerDiemLong = 369;
    ...
    this.setState({days, hours, perDiemDays}, () => this.calculateDiems());
  }
  ...
  print() {
    ...
    function redirect(state) {
      const url = `https://dash.variant.no/climate/registration?${state.name && `name=${state.name}&`}${state.mileage && state.mileage.mileage ? `mileage=${state.mileage.mileage}&` : ""}${state.departDate && `travelDate=${new Date(state.departDate).toISOString()}&`}`;
      window.location = url;
    }
  }
    ...
    return (<>
      <div className="container">
        ...
        <Refund
          ...
          overNight={this.state.overNight}
          handleOverNightToggle={this.handleOverNightToggle}
        />
        <Mileage
